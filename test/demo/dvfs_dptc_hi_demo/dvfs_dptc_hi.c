...
/*!
 * @file test/demo/dvfs_dptc_hi_demo/dvfs_dptc_hi.c
 *
 ...
 *
 */
void enable_dvfs(int dptc_fh)
{
  if(ioctl(dptc_fh,DVFS_IOCTENABLE) < 0)
		printf("command failed\n");
}
 ...
 *
 */
void disable_dvfs(int dptc_fh)
{
        if(ioctl(dptc_fh,DVFS_IOCTDISABLE) < 0)
		printf("command failed\n");
}
...
/*!
 * This function checks the state of the DVFS module (Enabled/Disabled)
 * The function sends a GET_STATE command to the DVFS daemon and waits for an
 * ack message that contains the DPCT module state in the message parameter.
 *
 * @param     sock	the communication socket file descriptor number.
 *
 ...
 *
 */
int dvfs_is_active(int dptc_fh)
{
        return ioctl(dptc_fh,DVFS_IOCGSTATE);
}
...
void set_sw_gp(unsigned long gp_value,int dptc_fh)
{
	if (ioctl(dptc_fh,DVFS_IOCSSWGP,(unsigned char)gp_value) < 0)
                printf("command failed\n");        
}

void set_wfi(unsigned long gp_value,int dptc_fh)
{
	if (ioctl(dptc_fh,DVFS_IOCSWFI,(unsigned char)gp_value) < 0)
                printf("command failed\n");        
}

void set_wp(unsigned int wp,int dptc_fh)
{
	if (ioctl(dptc_fh,DPTC_IOCSWP,wp) < 0)
                printf("command failed\n");
}

void show_freq(int dptc_fh)
{
        int res;

        res = ioctl(dptc_fh,PM_IOCGFREQ);
        if (res > 0) {
                printf("ARM frequency is %dMHz\n", res/1000000);
        }
        else {
                printf("command failed\n");
        }
...
void process_cmd(char cmd,int dptc_fh,int dptc_proc_fh)
{
	...
	case '5':
		...
		if(scanf("%x",&rc_status) == 1){
                        set_ref_circuits(rc_status,dptc_fh);
                }
                else{
                        printf("Failed scanf refcircuits\n");
                }
		break;
	...
	/* Disable DPTC module command */
	case '9':
		disable_dvfs(dptc_fh);
		break;
        /* Set SW gp values */
	case 'a':
                printf("\nSW gp values: ");
		if(scanf("%x",&rc_status) == 1){
                        set_sw_gp(rc_status,dptc_fh);
                }
                else{
                        printf("Failed scanf gp values\n");
                }
		break;
                /* Set SW gp values */
	case 'b':
                printf("\nWFI state: ");
		if(scanf("%x",&rc_status) == 1){
                        set_wfi(rc_status,dptc_fh);
                }
                else{
                        printf("Failed scanf wfi status\n");
                }
		break;
        case 'c':
                show_freq(dptc_fh);
...
void human_interface(int dptc_fh,int dptc_proc_fh)
{
	...
	while (cmd != '7') {
		...
		printf("Enabled - %d\n",dptc_is_active(dptc_fh));
                ...
                printf("DVFS module status:\n");
		printf("Enabled - %d\n\n",dvfs_is_active(dptc_fh));
		...
		//printf("5) Set new frequency index\n");
                ...
                printf("------------------------\n");
                printf("DVFS driver commands\n");
                printf("8) Enable DVFS\n");
                printf("9) Disable DVFS\n");
                printf("a) Set SW general purpose bits\n");
                printf("b) Set WFI monitor\n");
                printf("c) Show frequency\n");
                printf("------------------------\n");
                printf("------------------------\n");
		printf("q) Quit\n");
...
int main(int argc, char *argv[])
{
        ...
        if((dptc_fh = open(dev_name, O_RDWR))<0){
                printf("Failed open %s\n", dev_name);
                return -1;
        }
        
        
        if((dptc_proc_fh = open(proc_name, O_RDONLY))<0){
                 printf("Failed open %s\n", proc_name);
        }
