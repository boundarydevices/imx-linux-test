...
/*
 * @file mxc_spdc_fb_test.c
 *
 * @brief Mxc SPDC framebuffer driver unit test application
 *
 * Base on EPDC unit test.
 */
...
static void copy_image_to_buffer_byte(int left, int top, int width, int height, const unsigned char *img_ptr,
			uint target_buf, struct fb_var_screeninfo *screen_info)
{
	int i;
	__u32 *dest;
	...
	if (target_buf == BUFFER_FB)
		...
		fb_ptr = (uint *)fb +
			(screen_info->xres_virtual * (screen_info->yres) *
			screen_info->bits_per_pixel/8)/4;
	else {
		printf("Invalid target buffer specified!\n");
		return;
...
static int test_rotation(void)
{
	...
	for (i = FB_ROTATE_UR; i <= FB_ROTATE_CCW; i++) {
		printf("Blank screen\n");
		memset(fb, 0x00, screen_info.xres_virtual*screen_info.yres*screen_info.bits_per_pixel/8);
		update_to_display(0, 0, screen_info.xres, screen_info.yres,
			WAVEFORM_MODE_INIT, TRUE, 0);
		...
		update_to_display(0, 0, 480, 360, WAVEFORM_MODE_INIT, FALSE, 0);
		...
		draw_rgb_color_squares(&screen_info);
		update_to_display(50, 50, 200, 200, WAVEFORM_MODE_DU, TRUE, 0);
		update_to_display(350, 50, 200, 200, WAVEFORM_MODE_DU, TRUE, 0);
		update_to_display(50, 350, 200, 200, WAVEFORM_MODE_DU, TRUE, 0);
		update_to_display(350, 350, 200, 200, WAVEFORM_MODE_INIT, TRUE, 0);
		...
		for (j = 8; j >= 4; j--) {
			...
			update_to_display(j, j, 104 - j, 104 - j, WAVEFORM_MODE_INIT, TRUE, 0);
		}

		sleep(1);

		printf("Unrotated, changing to y4\n");
		...
		printf("Draw square outline (4-bit gray)\n");
		for (j = 8; j >= 0; j--) {
			memset(fb, 0x0, screen_info.xres_virtual*screen_info.yres*screen_info.bits_per_pixel/8);
			update_to_display(0, 0, screen_info.xres, screen_info.yres,
				WAVEFORM_MODE_DU, TRUE, 0);
		...
		for (j = 9; j <= 14; j++) {
			...
			update_to_display(j, j, 96, 96, WAVEFORM_MODE_INIT, TRUE, 0);
		}
...
static int test_y4(void)
{
	...
	/*
	 * Do Y4 FB test sequence twice:
	 * - once for normal Y4 (grayscale = 3)
	 * - once for inverted Y4 (grayscale = 4)
	 */
	for (iter = 3; iter < 5; iter++) {
		if (iter == GRAYSCALE_4BIT)
			printf("Changing to Y4 Framebuffer\n");
		else if (iter == GRAYSCALE_4BIT_INVERTED)
			printf("Changing to Inverted Y4 Framebuffer\n");
		...
		if (retval < 0)
		{
			printf("Rotate and go to Y4 failed\n");
			return TFAIL;
		}
		...
		update_to_display(0, 0, screen_info.xres, screen_info.yres,
			WAVEFORM_MODE_INIT, FALSE, 0);

		sleep(3);

		printf("Draw Y4 colorbar\n");
...
static int test_auto_update(void)
{
	...
	printf("Please enable AUTO update mode with deferred io Feature in Kernel\n");
...
static int test_pan(void)
{
	...
	for (i = 1; i <= 6; i++) {
		...
		if (retval < 0)
			printf("Pan fail!\n");
		update_to_display(100*i, 100, 80, 80, WAVEFORM_MODE_INIT, FALSE, 0);
	...
	for (i = 0; i < 4; i++) {
		...
		update_to_display(100, 200, 80, 80, WAVEFORM_MODE_INIT, FALSE, 0);
	}
...
static int test_animation_mode(void)
{
	...
	/*
	 * Do Y4 FB test sequence twice:
	 * - once for normal Y4 (grayscale = 1)
	 * - once for inverted Y4 (grayscale = 2)
	 */
	for (iter = 3; iter < 5; iter++) {
		if (iter == GRAYSCALE_4BIT)
			printf("Changing to Y4 Framebuffer\n");
		...
		draw_y_colorbar(&screen_info);
		printf("Draw Y4 colorbar normal\n");
		update_to_display(0, 0, screen_info.xres, screen_info.yres,
			WAVEFORM_MODE_GC16, TRUE, 0);

		sleep(2);

		printf("Draw Y4 colorbar black/white\n");
		update_to_display(0, 0, screen_info.xres, screen_info.yres,
			wave_mode, TRUE, EPDC_FLAG_FORCE_MONOCHROME);
...
static int test_shift(void) {
	...
	update_to_display(300, 169, 141, 109, WAVEFORM_MODE_INIT, FALSE, 0);

	printf("Update at 300,169\n");
	sleep(5);

	update_to_display(301, 200, 141, 109, WAVEFORM_MODE_INIT, FALSE, 0);
...
static int test_colormap(void)
{
	...
	/* Make update request using new colormap */
	update_to_display(0, 0, screen_info.xres, screen_info.yres,
		WAVEFORM_MODE_INIT, TRUE, EPDC_FLAG_USE_CMAP | EPDC_FLAG_ENABLE_INVERSION);
...
void usage(char *app)
{
	printf("SPDC framebuffer driver test program.\n");
	printf("Usage: mxc_spdc_fb_test [-h] [-a] [-p delay] [-u s/q/m] [-n <expression>]\n");
	printf("\t-h\t  Print this message\n");
	printf("\t-a\t  Enabled animation waveforms for fast updates (tests 7-8)\n");
	printf("\t-p\t  Provide a power down delay (in ms) for the SPDC driver\n");
	...
	printf("4 - Panning Updates\n");
	printf("5 - Overlay Updates\n");
	printf("6 - Auto-Updates\n");
	printf("7 - Animation Mode Updates\n");
	printf("8 - Fast Updates\n");
	printf("9 - Partial to Full Update Transitions\n");
	printf("10 - Test Pixel Shifting Effect\n");
	printf("11 - Colormap Updates\n");
	printf("12 - Stress Test\n");
}
...
main(int argc, char **argv)
{
	...
	while (1) {
		...
		/* If we found SPDC, exit loop */
		if (!strcmp(SPDC_STR_ID, screen_info_fix.id)) {
			printf("Opened SPDC fb device %s\n", fb_dev);
			break;
		}
	 ...
	 * to using it for SPDC ioctls
	 */
	fd_fb_ioctl = open("/dev/spdc_test", O_RDWR, 0);
	if (fd_fb_ioctl >= 0)
		printf("\n****Using SPDC kernel module test driver!****\n\n");
	else
		fd_fb_ioctl = fd_fb;
	...
	testfunc_array[2] = &test_y4;
	testfunc_array[3] = &test_pan;
	testfunc_array[4] = &test_overlay;
	testfunc_array[5] = &test_auto_update;
	testfunc_array[6] = &test_animation_mode;
	testfunc_array[7] = &test_fast_square;
	testfunc_array[8] = &test_partial_to_full;
	testfunc_array[9] = &test_shift;
	testfunc_array[10] = &test_colormap;
	testfunc_array[11] = &test_stress;
