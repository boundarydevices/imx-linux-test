...
#include "python_tutorial_0001_rgb_1024x758.c"
#include "python_tutorial_0002_rgb_1024x758.c"
#include "python_tutorial_0003_rgb_1024x758.c"
#include "python_tutorial_0004_rgb_1024x758.c"
...
int use_animation = 0;
int num_flashes = 10;
static int use_reagl;
static int use_reagld;
...
static __u32 update_to_display_with_dithering(int left, int top, int width, int height, int wave_mode,
	int wait_for_complete, uint flags, struct hw_dithering *hw_dithering)
{
	struct mxcfb_update_data upd_data;
	struct mxcfb_update_marker_data upd_marker_data;
	int retval;
	...
	upd_data.waveform_mode = wave_mode;
	if (hw_dithering) {
		upd_data.dither_mode = hw_dithering->dither_mode;
		upd_data.quant_bit = hw_dithering->quant_bit;
	}
	upd_data.update_region.left = left;
	upd_data.update_region.width = width;
	upd_data.update_region.top = top;
...
static int test_updates(void)
{
	...
	if (!use_reagl && !use_reagld)
		wave_mode = WAVEFORM_MODE_AUTO;
	printf("wave_mode %d\n", wave_mode);

	/* Update each square */
	update_to_display(50, 50, 200, 200, wave_mode, TRUE, 0);
	update_to_display(350, 50, 200, 200, wave_mode, TRUE, 0);
	update_to_display(50, 350, 200, 200, wave_mode, TRUE, 0);
	update_to_display(350, 350, 200, 200, wave_mode, TRUE, 0);
	...
	update_to_display(300, 0, 480, 560, wave_mode, TRUE, 0);
	...
	update_to_display(0, 0, 800, 600, wave_mode, FALSE, 0);
...
static int test_pan(void)
{
	...
	for (i = 1; i <= 6; i++) {
		...
		if (retval < 0)
			printf("Pan fail!\n");
		update_to_display(100*i, 100, 80, 80, wave_mode, FALSE, 0);
	...
	for (i = 0; i < 4; i++) {
		update_to_display(100, 200, 80, 80, wave_mode, FALSE, EPDC_FLAG_ENABLE_INVERSION);
		update_to_display(100, 200, 80, 80, wave_mode, FALSE, 0);
	}
	...
	for (i = 0; i < 4; i++) {
		...
		if (retval < 0)
			printf("Pan fail!\n");
		update_to_display(0, 0, 80, 80, wave_mode, FALSE, 0);
		usleep(100);
...
static int test_animation_mode(void)
{
	...
	if (!use_animation && !use_reagl && !use_reagld)
		wave_mode = WAVEFORM_MODE_AUTO;
	printf("wave_mode %d\n", wave_mode);
...
static int test_colormap(void)
{
	...
	update_to_display(0, 0, screen_info.xres, screen_info.yres,
		wave_mode, TRUE, EPDC_FLAG_USE_CMAP);
	...
	update_to_display(0, 0, screen_info.xres, screen_info.yres,
		wave_mode, TRUE, EPDC_FLAG_USE_CMAP | EPDC_FLAG_ENABLE_INVERSION);
...
static int test_dry_run(void)
{
	...
	coll = update_to_display(0, 0, 300, 250, wave_mode, FALSE, EPDC_FLAG_TEST_COLLISION);
	if (coll)
		retval = TFAIL;
	update_to_display(0, 0, 300, 250, wave_mode, FALSE, 0);
	usleep(4000);

	/* Draw overlapping rectangle */
	draw_rectangle(fb, 250, 200, 300, 250, 0x4104);
	update_to_display(250, 200, 300, 250, wave_mode, FALSE, 0);
	...
	draw_rectangle(fb, 250, 200, 300, 250, 0x4104);
	coll = update_to_display(250, 200, 300, 250, wave_mode, FALSE, EPDC_FLAG_TEST_COLLISION);
	if (coll)
		sleep(1);
	else
		retval = TFAIL;
...
static int test_hw_dithering(void)
{
	int wave_mode;
	...
	printf("Ginger dithering mode \"%s\" and quant_bit %d\n",
		((hw_dithering.dither_mode >= 0) && (hw_dithering.dither_mode <
		ARRAY_SIZE(dithering_name))) ? dithering_name[hw_dithering.dither_mode] : "Unsupported",
		hw_dithering.quant_bit);
	copy_image_to_buffer(0, 0, 800, 600, ginger_rgb_800x600,
		BUFFER_FB, &screen_info);

	update_to_display_with_dithering(0, 0, 800, 600, wave_mode, TRUE,
		0, &hw_dithering);
	sleep(2);
	return 0;
}
...
static int test_aa(void)
{
	...
	for (wave_mode=3; wave_mode < 6; wave_mode++)
	{
		printf("Blank screen\n");
		memset(fb, 0xFF, screen_info.xres_virtual*screen_info.yres*screen_info.bits_per_pixel/8);
		full_update_to_display(WAVEFORM_MODE_GC16, TRUE, 0);
		sleep(1);

		printf("Waveform mode: %i\n", wave_mode);
		for (img=1; img < 5; img++)
		{
			switch (img)
			{
				case 1: imgptr = xga_python_tutorial_0001_rgb_1024x758; break;
				case 2: imgptr = xga_python_tutorial_0002_rgb_1024x758; break;
				case 3: imgptr = xga_python_tutorial_0003_rgb_1024x758; break;
				case 4: imgptr = xga_python_tutorial_0004_rgb_1024x758; break;
			}

			copy_image_to_buffer(0, 0, 1024, 758, imgptr, BUFFER_FB, &screen_info);
			full_update_to_display(wave_mode, TRUE, 0);
...
void usage(char *app)
{
	...
	printf("\t-r\t  Enabled REAGL updates)\n");
	printf("\t-g\t  Enabled REAGL-D updates\n");
	...
	printf("\t-d\t  Execute the hardware dithering test\n");
	printf("\t\t  0 - Pass Through\n");
	printf("\t\t  1 - Floyd-Steinberg\n");
	printf("\t\t  2 - Atkinson\n");
	printf("\t\t  3 - Ordered\n"),
	printf("\t\t  4 - No Dithering, quantization only\n");
	printf("\t-q\t  quantization bit (1 - 7) \n");
	printf("Example:\n");
	printf("%s -n 1-3,5,7\n", app);
	printf("\nEPDC tests:\n");
...
main(int argc, char **argv)
{
	...
	while ((rt = getopt(argc, argv, "hargu:w:n:d:q:p:f:")) >= 0) {
		switch (rt) {
		case 'h':
			usage(argv[0]);
			return TPASS;
		...
		case 'n':
			...
			break;
		case 'd':
			hw_dithering.dither_mode = atoi(optarg);
			break;
		case 'q':
			hw_dithering.quant_bit = atoi(optarg);
			printf("quantbit %d   ==>\n", hw_dithering.quant_bit);
			break;
		case 'p':
