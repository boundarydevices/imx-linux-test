                      ...
                      uint32_t len, uint8_t* encrypt_output)
{
    ...
    if (decrypt_output != NULL) {
        code = fsl_shw_establish_key(my_ctx, key_info, FSL_KEY_WRAP_CREATE,
                                     NULL);
        if (code != FSL_RETURN_OK_S) {
            ...
            if (code != FSL_RETURN_OK_S) {
                printf("fsl_shw_symmetric_encrypt() returned: %s\n",
                       fsl_error_string(code));
            } else {
                ...
                code = fsl_shw_symmetric_decrypt(my_ctx, key_info, sym_ctx,
                                                 ...
                                                 decrypt_output);
                if (code != FSL_RETURN_OK_S) {
                    printf("fsl_shw_symmetric_decrypt() returned: %s\n",
                           fsl_error_string(code));
                } else {
                                   ...
                                   uint32_t len)
{
    ...
    if (decrypt_output != NULL) {
        ...
        if (code != FSL_RETURN_OK_S) {
            ...
            if (err_err != 0) {
                printf("Warning: could not release key with handle 0x%x: %s\n",
                       handle, fsl_error_string(err_err));
            }
        } else {
            init_key_and_sym_ctx(key_alg, key_len, &new_key_info, sym_ctx);
            ...
            if (code != FSL_RETURN_OK_S) {
                printf("fsl_shw_establish_key(UNWRAP) returned: %s\n",
                       fsl_error_string(code));
            } else {
                fsl_shw_return_t err_err;
                ...
                if (err_err != 0) {
                    printf("Warning: could not release key with handle 0x%x: "
                           "%s\n",
                           handle, fsl_error_string(err_err));
                }
            }
              ...
              uint32_t* total_failed_count)
{
    ...
    if ((cap == NULL) || !fsl_shw_pco_check_black_key_supported(cap)) {
        ...
        || (sym_ctx == NULL)) {
        printf("Memory allocation problems. Skipping wrapped Key Tests\n");
        *total_failed_count += 4;
        ...
        printf("Secret Key Test %d: Generate and use a random %d-byte RED"
               " key\n", testno, key_len);
        ...
        for (i = 0; i < 2; i++) {
            ...
            printf("Secret Key Test %d: Establish a known %u-byte RED key\n",
                   testno, key_len);
            passed = 0;

            code = fsl_shw_establish_key(my_ctx, &key_info,
                                         FSL_KEY_WRAP_ACCEPT, known_key);
            if (code != FSL_RETURN_OK_S) {
                printf("fsl_shw_establish_key(ACCEPT) returned: %s\n",
                       fsl_error_string(code));
            } else {
                ...
                if (code != FSL_RETURN_OK_S) {
                    ...
                    if (! compare_result(decrypt_input, encrypt_output,
                                         sizeof(known_plaintext),
            ...
            printf("Secret Key Test %d: Re-establish known %u-byte RED key\n",
                       testno, key_len);
            passed = 0;
            ...
            if (blob == NULL) {
                ...
                if (code != FSL_RETURN_OK_S) {
                    ...
                    if (err_err != 0) {
                        printf("Warning: could not release key with handle "
                               "0x%x: %s\n",
                               handle, fsl_error_string(err_err));
                    }
                } else {
                    if (key_alg == FSL_KEY_ALG_ARC4) {
                        fsl_shw_scco_init(sym_ctx, key_alg,
                                          FSL_SYM_MODE_STREAM);
                        fsl_shw_scco_set_flags(sym_ctx, FSL_SYM_CTX_INIT);
                    } else {
                        if ((key_alg == FSL_KEY_ALG_TDES)
                            || (key_alg == FSL_KEY_ALG_DES)) {
                            fsl_shw_sko_set_flags(&key_info,
                    ...
                    if (code != FSL_RETURN_OK_S) {
                        ...
                        code =
                            fsl_shw_symmetric_decrypt(my_ctx, &key_info,
                                                      ...
                                                      decrypt_output);
                        if (code != FSL_RETURN_OK_S) {
                            printf("fsl_shw_symmetric_decrypt() returned:"
                                   " %s\n", fsl_error_string(code));
                        } else {
                            if (! compare_result(known_plaintext,
                                                 decrypt_output,
                                                 sizeof(known_plaintext),
                                                 "decrypted plaintext")) {
                                passed = 1;
